...
// ------------------ Босс ------------------
async function deleteRecord(id){ await supabase.from('records').delete().eq('id',id); await fetchRecords(); renderRecords(); }
function showBossEarnings(){
  ...
  if(period==='week'){const start=new Date(now); start.setDate(now.getDate()-now.getDay()); filtered=filtered.filter(r=>new Date(r.date)>=start && new Date(r.date)<=now);}
  if(period==='month') filtered=filtered.filter(r=>{const d=new Date(r.date); return d.getMonth()===now.getMonth() && d.getFullYear()===now.getFullYear();});
...
async function markRecord(id,status){
  ...
  if(status==='Сделано'){ const earnedStr=prompt('Введите сумму за услугу (грн):'); const parsed=parseFloat(earnedStr); if(!isNaN(parsed)) earned=parsed; }
  await supabase.from('records').update({status,earned,addedBy:currentUser.login}).eq('id',id);
  await fetchRecords(); renderRecords();
}
async function addWork(id){
  const rec=records.find(r=>r.id===id);
  if(!rec) return;
  const earnedStr=prompt('Введите сумму за услугу (грн):'); const parsed=parseFloat(earnedStr);
  if(isNaN(parsed)) return;
  await supabase.from('records').update({earned:parsed}).
